...
Mtx44::Mtx44(float a00, float a10, float a20, float a30, float a01, float a11, float a21, float a31, float a02, float a12, float a22, float a32, float a03, float a13, float a23, float a33){ //Default constructor, default values are zero, params (Initial values for the matrix in row-column format)
	a[0] = a00;
	a[1] = a10;
	a[2] = a20;
	a[3] = a30;
...
Mtx44::Mtx44(const float m[16]){ //Default values are zero, param m[16] (Initial values for the matrix in column major format)
	for(int i = 0; i < 16; ++i){
		a[i] = m[i];
	}
}

Mtx44::Mtx44(const Mtx44 &RHS){ //Copy constructor
	for(int i = 0; i < 16; ++i){
		a[i] = RHS.a[i];
	}
}
